...
# 결과 출력
answer=filtered_contours[0]
answer = answer.astype(np.float64)
answer[:, 0, 0] *= image.shape[0]/800
answer[:, 0, 1] *= image.shape[1]/600
answer = answer.astype(np.int32)
cv2.drawContours(image, [answer], -1, (255,0,255), 2)
mask = np.zeros_like(image)
# 모든 컨투어를 마스크에 그리기
cv2.drawContours(mask, [answer], -1, (255, 255, 255), thickness=cv2.FILLED)
