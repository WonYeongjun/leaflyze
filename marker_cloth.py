...
if __name__ == "__main__":
    ...
    # 모든 컨투어를 마스크에 그리기
    cv2.drawContours(
        mask, [contour_rescaled], -1, (255, 255, 255), thickness=cv2.FILLED
    )
    ...
    for i, side in enumerate(sides):
        ...
        cv2.circle(
            sides[i], (int(image_xy[i][0]), int(image_xy[i][1])), 5, (0, 255, 0), -1
        )
        real_circle.append((int(kimage_xy[i][0]), int(kimage_xy[i][1])))
        cv2.circle(
            sides[i],
            (int(kimage_xy[i][0]), int(kimage_xy[i][1])),
            20,
            (0, 255, 255),
            -1,
        )
    ...
    bottom_right = cv2.resize(
        bottom_right,
        (int(bottom_right.shape[1] * 0.2), int(bottom_right.shape[0] * 0.2)),
    )
